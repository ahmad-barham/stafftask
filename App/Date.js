...
import { StyleSheet, ActivityIndicator, View, Text, Alert ,Button, SafeAreaView,TouchableOpacity,
TextInput
   } from 'react-native';
...
if(selectstartDate!="Invalid date" &&selectendDate!="Invalid date"){
  ...
  customStyleIndex===0?  <Text>{moment(singleDate).format('DD-MM-YYYY')}</Text>:
  <View><Text>{selectstartDate}</Text>

  <Text>{selectendDate}</Text></View>
}
